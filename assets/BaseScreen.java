...
import com.seedstir.constants.Constants;
import com.seedstir.network.HttpClient;
import com.seedstir.settings.Appsettings;
import com.seedstir.settings.CompanyInfo;
import com.seedstir.settings.LoginInfo;
import com.seedstir.settings.UserInfo;
...
public class BaseScreen extends Activity implements OnClickListener{
	...
	protected void onStart() {
		...
		if(!app.init){
			...
			app.setCompanyinfo(new CompanyInfo(this));
			...
			if(result != null){
				...
				if(status_code == 1){
					if(obj.getString("type").equalsIgnoreCase("1")){
						...
						app.getCompanyinfo().SetCompanyInfo(email, user_id, profileuserID, company_name, logo, company_desc,address,company_type);
						app.getLoginfo().setLoginInfo(username, password);
